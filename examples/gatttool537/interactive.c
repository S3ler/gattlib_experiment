...
static void cmd_write_tx_notify_hnd() {
    ...
    if (conn_state != STATE_TX_VALUE_SAVED) {
        failed("Not STATE_TX_VALUE_SAVED\n");
        return;
    }
...
static void cmd_read_tx_buffer_cb(guint8 status, const guint8 *pdu, guint16 plen,
    ...
    if (conn_state != STATE_RX_VALUE_CLEARING) {
        failed("Not STATE_RX_VALUE_CLEARING\n");
        return;
    }
...
static void cmd_read_tx_buffer() {
    ...
    if (conn_state != STATE_HANDLE_READY) {
        failed("Not STATE_HANDLE_READY\n");
        return;
    }

    handle = nus_rx_handle;
    if (handle < 0) {
        error("Invalid handle: 0x%0x4\n", handle);
        return;
    }
    conn_state = STATE_RX_VALUE_CLEARING;
    gatt_read_char(attrib, handle, cmd_read_tx_buffer_cb, attrib);
}
...
static void char_desc_cb(uint8_t status, GSList *descriptors, void *user_data) {
    ...
    if (conn_state != STATE_HANDLE_CHECKING) {
        error("Invalid State - must be STATE_HANDLE_CHECKING\n");
        return;
    }
        ...
        if (!done) {
            if (strcmp(desc->uuid, TX_CHRC_UUID) == 0) {
                if (!hnd_tx) {
                    ...
                    error("Discovered duplicate hnd_tx");
                    return;
                }
            }
            if (strcmp(desc->uuid, RX_CHRC_UUID) == 0) {
                if (!hnd_rx) {
                    ...
                    error("Discovered duplicate hnd_rx");
                    return;
                }
            }
            if (!hnd_rx_cccd && hnd_rx && strcmp(desc->uuid, RX_CCCD_UUID) == 0) {
                if (!hnd_rx_cccd) {
                    ...
                    error("Discovered duplicate hnd_rx_cccd");
                    return;
                }
            }
    ...
    if (done) {
        ...
        error("Did not discover necessary characteristics and CCCDs");
    }
}
...
static void connection_state_machine() {
...
static void parse_line(char *line_read) {
    ...
    if (strlen(line_read) > 9) {
        printf("Message is longer then 10 bytes.\n");
        goto done;
    }
    ...
    char *argvp_array[2 + 10] = {0};
    // "char-write_req 0xRX_HDN 05 04 03 02 01 00"
    // FIXME adept argcp, argvp
    argcp = 10;
    argvp_array[0] = "char-write-req";
    char argvp_rx_handle[20] = {0};
    sprintf(argvp_rx_handle, "0x%0x4", RX_HND);
    argvp_array[1] = argvp_rx_handle;
#define s_c_l  3
    char c_a[s_c_l], c_b[s_c_l], c_c[s_c_l], c_d[s_c_l], c_e[s_c_l],
            c_f[s_c_l], c_g[s_c_l], c_h[s_c_l], c_i[s_c_l], c_j[s_c_l] = {0};
    argvp_array[2 + 0] = c_a;
    argvp_array[2 + 1] = c_b;
    argvp_array[2 + 2] = c_c;
    argvp_array[2 + 3] = c_d;
    ...
    for (int i = 0; i < 10 || line_read[i] == '\n'; i++) {
        sprintf(argvp_array[2 + i], "%0x2", line_read[i]);
    }

    argvp = argvp_array;
    conn_state = STATE_RX_WRITE_REQ;
    // write
    cmd_char_write(argcp, argvp);
    /*
    if (*line_read == '\0'){}
...
int interactive(const char *src, const char *dst,
    ...
    char *fake_argvp[2];
    char connect_command[] = "connect";
    char connect_mac[] = "00:1A:7D:DA:71:11";
